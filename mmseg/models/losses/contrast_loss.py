...
class PixelContrastLoss(nn.Module, ABC):
    ...
    def _hard_anchor_sampling(self, X, y_hat, y):
        ...
        for ii in range(batch_size):
            ...
            #     if (this_y == x).nonzero().shape[0] < min_view:
            #         min_view = (this_y == x).nonzero().shape[0]
        ...
        # n_view = min(n_view, min_view)

        X_ = torch.zeros((total_classes, n_view, feat_dim), dtype=torch.float).cuda(self.device)
        y_ = torch.zeros(total_classes, dtype=torch.float).cuda(self.device)
        ...
        for ii in range(batch_size):
            ...
            for cls_id in this_classes:
                ...
                if num_hard >= n_view / 2 and num_easy >= n_view / 2:
                    ...
                    # .info('this shoud be never touched! {} {} {}'.format(num_hard, num_easy, n_view))
                    raise Exception
    ...
    def _contrastive(self, feats_, labels_):
        ...
        mask = torch.eq(labels_, torch.transpose(labels_, 0, 1)).float().cuda(self.device)
        ...
        logits_mask = torch.ones_like(mask).scatter_(1,
                                                     torch.arange(anchor_num * anchor_count).view(-1, 1).cuda(self.device),
                                                     0)
    ...
    def forward(self, feats, labels=None, predict=None, **kwargs):
        ...
        loss = self.loss_weight * self._contrastive(feats_, labels_)
        return loss
